...
read.hxsurf<-function(filename,RegionNames=NULL,RegionChoice="Inner",
  ...
  for(i in 1:nPatches){
    ...
    for (choice in 1:length(RegionChoice)) {
      RegionName=getfield(paste(RegionChoice[choice],"Region",sep=""),PatchHeader,2)
      # Don't add the Exterior region
      if (RegionChoice[choice] == "Outer" && RegionName == "Exterior") next
