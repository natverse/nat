...
## Load Data
We're going to look at a data set of olfactory projection neurons originally 
...
The `load` statement above resulted in a `neuronlist` object called `Cell07PNs` 
...
```{r}
n=Cell07PNs[['CT12T']]
all.equal(n, Cell07PNs[[2]])
```
### Neuron internals

```{r}
# overview of neuron object
str(Cell07PNs[[1]], max=1)
...
dl1n=subset(Cell07PNs, Glomerulus=="DL1")

```

## Iterating over the neurons in a neuronlist
...
rval=plot3d(Cell07PNs, subset=grepl("^D", Glomerulus), col=factor(Glomerulus),
