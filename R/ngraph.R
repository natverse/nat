...
ngraph<-function(el, vertexlabels, xyz=NULL, diam=NULL, directed=TRUE,
  ...
  g=make_graph_nat(rawel, n=length(vertexlabels), directed=directed)
...
spine <- function(n, UseStartPoint=FALSE, SpatialWeights=TRUE, invert=FALSE,
  ...
  if(UseStartPoint) {
    ...
    lps= distances(graph = ng, n$StartPoint, to = n$EndPoints,
                       mode = 'all')
    ...
    if(rval=='length') {
      ...
      longestpath=get_diameter(ng, directed=FALSE)
    }
  }
  if(rval=='ids') {
    if(invert) {
